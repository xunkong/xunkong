...
namespace Xunkong.Desktop.Services
{
    ...
    public class UserSettingService
    {
        ...
        public T? GetSetting<T>(string key, bool notPrintLog = false)
        {
            using var con = _cntFactory.CreateDbConnection();
            var value = con.QueryFirstOrDefault<string>($"SELECT Value FROM UserSettings WHERE Key=@Key;", new { Key = key });
            if (!notPrintLog)
            {
                _logger.LogTrace("Query UserSetting by key {Key} with value {Value}", key, value);
            }
        ...
        public void SaveSetting<T>(string key, T value, bool notPrintLog = false)
        {
            if (!notPrintLog)
            {
                _logger.LogTrace("Save UserSetting with key {Key}, value {Value}", key, value);
            ...
            try
            {
                ...
                con.Execute("INSERT OR REPLACE INTO UserSettings VALUES (@Key,@Value);", setting);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Save UserSetting.");
