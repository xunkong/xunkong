...
internal static class PreCacheService
{
    ...
    private static async void PreCacheAllFilesAsync(CancellationToken token = default)
    {
        ...
        {
            IsRunning = true;
            var folder = (await XunkongCache.Instance.GetCacheFolderAsync()).Path;
            ...
            if (list.Any())
            {
                ...
                {
                    var file = await XunkongCache.Instance.GetFromCacheAsync(new Uri(url), cancellationToken: t);
                    ulong size = 0;
                    if (file is not null)
                    {
    ...
    private static List<string> GetAllImages()
    {
        ...
        return list.Where(x => !string.IsNullOrWhiteSpace(x)).Distinct().Where(x => !File.Exists(XunkongCache.Instance.GetCacheFilePath(new Uri(x!)))).ToList()!;
    }
