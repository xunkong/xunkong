...
public sealed partial class SettingPage : Page
{
    ...
    private async Task UpdateGenshinDataAsync()
    {
        try
        {
            ...
            UpdateGenshinDataText = $"数据量较大，请耐心等待（感谢来自 wangdage12 的元数据）。上次更新：刚刚";
    ...
    private static string GetUpdateGenshinDataText()
    {
        try
        {
            if (File.Exists(DatabaseProvider.GenshinDataDbPath))
            {
                ...
                if (time.Days > 0)
                {
                    return $"数据量较大，请耐心等待（感谢来自 wangdage12 的元数据）。上次更新：{time.Days}天前";
                }
                else if (time.Hours > 0)
                {
                    return $"数据量较大，请耐心等待（感谢来自 wangdage12 的元数据）。上次更新：{time.Hours}小时前";
                }
                else if (time.Minutes > 0)
                {
                    return $"数据量较大，请耐心等待（感谢来自 wangdage12 的元数据）。上次更新：{time.Minutes}分钟前";
                }
                else
                {
                    return $"数据量较大，请耐心等待（感谢来自 wangdage12 的元数据）。上次更新：刚刚";
                }
            }
            else
            {
                return $"数据量较大，请耐心等待（感谢来自 wangdage12 的元数据）";
            }
        }
        catch
        {
